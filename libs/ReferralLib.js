...
function setLinkPrefix(value){
  Bot.setProperty(LIB_PREFIX + 'refList_link_prefix', value, 'string')
}

function getPrefix(){
  if(trackOptions.linkPrefix){
    // prop getting is slowly here
    return trackOptions.linkPrefix
  }

  return Bot.getProperty(LIB_PREFIX + 'refList_link_prefix');
}

function trackRef(){
  let prefix = 'user'

  let uprefix = getPrefix();
